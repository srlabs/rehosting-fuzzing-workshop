...
static void __get_handle() {
  ...
  if (getenv("DESOCK_PORT")) {
    port = atoi(getenv("DESOCK_PORT"));
  }
  if (getenv("DESOCK_FORK")) {
    do_fork = true;
  }
  if (getenv("DESOCK_CLOSE_EXIT")) {
    do_close = true;
  }
  if (debug)
    fprintf(stderr, "DESOCK: initialized!\n");
}
...
int shutdown(int socket, int how) {
  ...
  if (!o_shutdown) {
    o_shutdown = dlsym(handle, "shutdown");
  }
...
int close(int socket) {
  ...
  if (listen_fd != -1 && socket == listen_fd) {
    if (debug)
      fprintf(stderr, "DESOCK: close bind\n");
    listen_fd = -1;
  }
  if (!handle) {
    __get_handle();
...
int fork() {
  if (do_fork) {
    if (debug)
      fprintf(stderr, "DESOCK: fake fork\n");
    return 0;
  }
  if (!handle) {
    __get_handle();
